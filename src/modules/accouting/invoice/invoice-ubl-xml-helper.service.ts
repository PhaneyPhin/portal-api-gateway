...
export class InvoiceUBLXMLHelperService {
  ...
  parseXML(xml: string): any {
    ...
    const taxSubtotalsArray = Array.isArray(taxSubtotals)
      ? taxSubtotals
      : [taxSubtotals];
    ...
    const result = {
      ...
      reference_document_number: nsSafe(
        invoice["cac:BillingReference"]?.["cac:InvoiceDocumentReference"]?.[
          "cbc:ID"
        ]
      ),
      ...
      invoice_type:
        this.MAP_TYPE_CODE[invoice[`cbc:${this.DOCUMENT_TYPE_CODE}`]],
      supplier: {
        endpoint_id: nsSafe(
      ...
      prepaid_payment: {
        paid_amount: Number(
          invoice["cac:PrepaidPayment"]?.["cbc:PaidAmount"]?._text || 0
        ),
      },
      allowance_charges: headerAllowance
        ? [
            {
              charge_indicator:
                headerAllowance["cbc:ChargeIndicator"] === "true",
      ...
      tax_total: {
        tax_amount: Number(
          invoice["cac:TaxTotal"]?.["cbc:TaxAmount"]?._text || 0
        ),
        tax_subtotals: taxSubtotalsArray.map((subtotal: any) => {
          return {
            ...
            tax_category: {
              ...
              percent: Number(
                subtotal["cac:TaxCategory"]?.["cbc:Percent"] || 0
              ),
              tax_scheme:
      ...
      legal_monetary_total: {
        ...
        tax_exclusive_amount: getNumericValue(
          legalMonetaryTotal["cbc:TaxExclusiveAmount"]
        ),
        tax_inclusive_amount: getNumericValue(
          legalMonetaryTotal["cbc:TaxInclusiveAmount"]
        ),
        allowance_total_amount: getNumericValue(
          legalMonetaryTotal["cbc:AllowanceTotalAmount"]
        ),
        prepaid_amount: getNumericValue(
          legalMonetaryTotal["cbc:PrepaidAmount"]
        ),
        payable_amount: getNumericValue(
      ...
      invoice_lines: (Array.isArray(invoice[`cac:${this.DOCUMENT_LINE}`])
        ...
        const lineTaxSubtotals =
          line["cac:TaxTotal"]?.["cac:TaxSubtotal"] || [];
        const lineTaxSubtotalsArray = Array.isArray(lineTaxSubtotals)
          ? lineTaxSubtotals
          : [lineTaxSubtotals];
        return {
          id: line["cbc:ID"] || null,
          ...
          allowance_charges: Array.isArray(lineAllowance)
            ? lineAllowance.map((allowance: any) => ({
            ...
            : [],
          tax: {
            tax_amount: getNumericValue(
              line["cac:TaxTotal"]?.["cbc:TaxAmount"]
            ),
            tax_subtotals: lineTaxSubtotalsArray.map((subtotal: any) => ({
              ...
              tax_category: {
                id: subtotal["cac:TaxCategory"]?.["cbc:ID"] || null,
                percent: getNumericValue(
                  subtotal["cac:TaxCategory"]?.["cbc:Percent"]
                ),
                tax_scheme:
                  subtotal["cac:TaxCategory"]?.["cac:TaxScheme"]?.["cbc:ID"] ||
                  null,
